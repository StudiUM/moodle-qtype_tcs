...
class qtype_tcs_edit_form extends question_edit_form {
    ...
    protected function definition_inner($mform) {
        $mform->addElement('editor', 'hypothisistext', get_string('hypothisistext', 'qtype_tcs'), array('rows' => 5), $this->editoroptions);
    ...
    protected function data_preprocessing($question) {
        ...
        $question->hypothisistext['format'] = empty($question->options->hypothisistextformat) ? editors_get_preferred_format() : $question->options->hypothisistextformat;
        ...
        $question->effecttext['format'] = empty($question->options->effecttextformat) ? editors_get_preferred_format() : $question->options->effecttextformat;
        $question->effecttext['itemid'] = $draftid;
        
        $question->labeleffecttext = $question->options->labeleffecttext;
        $question->labelhypothisistext = $question->options->labelhypothisistext;
        $question->showquestiontext = $question->options->showquestiontext;
    ...
    public function validation($data, $files) {
        ...
        if ($answercount < 5) {
            ...
            $errors['answer[2]'] = get_string('notenoughanswers', 'qtype_tcs', 2);
            $errors['answer[3]'] = get_string('notenoughanswers', 'qtype_tcs', 2);
            $errors['answer[4]'] = get_string('notenoughanswers', 'qtype_tcs', 2);
        }
